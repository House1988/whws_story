...
local f_quit_game_panel = class("f_quit_game_panel", function()
    return fc.FContainerPanel:create()
end)


-------------------------------------------------------------------------------
-- @function [parent=#f_quit_game_panel] ctor
-- 构造函数
-------------------------------------------------------------------------------
function f_quit_game_panel:ctor()
    self:registerNodeEvent()   
  	...
  	g_resource.g_director:getRunningScene():add_panel_to_layer(self.m_autoPanel, 6)	
	...
	g_resource.g_director:pause()
end

-------------------------------------------------------------------------------
-- @function [parent=#f_quit_game_panel] onEnter
-- on, 初始化事件监听
-------------------------------------------------------------------------------
function f_quit_game_panel:onEnter()    
end


function f_quit_game_panel:onExit()    
    self:unregisterNodeEvent()
end

function f_quit_game_panel:onEnterTransitionFinish()
end

function f_quit_game_panel:onExitTransitionStart()
end

function f_quit_game_panel:onCleanup()
end


function f_quit_game_panel:registerNodeEvent(handler)
    if not handler then
...
-------------------------------------------------------------------------------
-- @function [parent=#f_quit_game_panel] unregisterNodeEvent
-------------------------------------------------------------------------------
function f_quit_game_panel:unregisterNodeEvent()
    self:unregisterScriptHandler()
end


function f_quit_game_panel:createLayoutPanel()
	...
	local pop_cancle_button = fc.FScaleButton:create()
    pop_cancle_button:setSize(cc.size(144,53))
    pop_cancle_button:setAnchorPoint(cc.p(0.5,0.5))
    pop_cancle_button:setButtonImage("nobatch/putonganniu1_up.png")
    pop_bottom_panel:appendComponent(pop_cancle_button)
    pop_cancle_button:setPositionInContainer(cc.p(172,255.5))
    table_public_network_error_pop["pop_cancle_button"] = pop_cancle_button
    ...
    pop_cancle_button:appendComponent(pnep_qu)
    ...
    pop_cancle_button:appendComponent(pnep_xiao)
    ...
    pop_queding_button:setPositionInContainer(cc.p(362,255.5))
...
-------------------------------------------------------------------------------
-- @function [parent=#f_quit_game_panel] createCommonTips
-- 创建一般提示窗体，只有确定按钮
-------------------------------------------------------------------------------
function f_quit_game_panel:createCommonTips()	
    ...
    panel:setPositionInContainer(cc.p(self.m_real_size.width/2, self.m_real_size.height/2))
    
    self.m_noticeTable["pnep_pop_notice_label"]:setString(self.m_show_text)
    
    -- 取消按钮
    local cancelBtnCallback = function()
		self:hide()
    	g_game.g_sdkManager.m_quit_game_panel = nil
	end
	self.m_noticeTable["pop_cancle_button"]:addHandleOfcomponentEvent(cancelBtnCallback, g_resource.g_f_touch_event.F_TOUCH_UPINSIDE)
	
	-- 确定按钮
	local noticeYes = function()
		g_game.g_gameManager:exitGame()
	end
...
-------------------------------------------------------------------------------
-- @function [parent=#f_quit_game_panel] setShowText
--设置显示面板中文本
-------------------------------------------------------------------------------
function f_quit_game_panel:setShowText(text)
	self.m_show_text = text
end

-------------------------------------------------------------------------------
-- @function [parent=#f_quit_game_panel] hide
--隐藏面板
-------------------------------------------------------------------------------
function f_quit_game_panel:hide()
	g_resource.g_director:resume()
	local layer = g_resource.g_director:getRunningScene():get_layer_from_config(6)
	layer:removeChild(self.m_autoPanel, true)
end

return f_quit_game_panel
