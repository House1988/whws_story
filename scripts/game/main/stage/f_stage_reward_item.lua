--[[
	继承自container panel, 用于显示过关奖励礼包
]]--

require("scripts.auto.auto_attachmentItem")

local f_stage_reward_item = class("f_stage_reward_item", function()
    return g_game.g_classFactory.newFLuaContainerPanel("f_stage_reward_item")
end)


-------------------------------------------------------------------------------
-- @function [parent=#f_stage_reward_item] ctor
-- 构造函数
-------------------------------------------------------------------------------
function f_stage_reward_item:ctor()    
...
-------------------------------------------------------------------------------
-- @function [parent=#f_stage_reward_item] onEnter
-- on, 初始化事件监听
-------------------------------------------------------------------------------
function f_stage_reward_item:onEnter() 
	
end


function f_stage_reward_item:onExit()    
    self:unregisterNodeEvent()
end

function f_stage_reward_item:onEnterTransitionFinish()
end

function f_stage_reward_item:onExitTransitionStart()
end

function f_stage_reward_item:onCleanup()
end


function f_stage_reward_item:registerNodeEvent(handler)
    if not handler then
...
function f_stage_reward_item:unregisterNodeEvent()
    self:unregisterScriptHandler()
end

-------------------------------------------------------------------------------
-- @function [parent=#f_stage_reward_item] setSilver
-------------------------------------------------------------------------------
function f_stage_reward_item:setSilver(silver)
...
-------------------------------------------------------------------------------
-- @function [parent=#f_stage_reward_item] setGold
-------------------------------------------------------------------------------
function f_stage_reward_item:setGold(gold)
	local frame = self.m_componentTable["frame"]
	local p = frame:getPositionInContainer()
	local icon = self.m_componentTable["image"]
...
-------------------------------------------------------------------------------
-- @function [parent=#f_stage_reward_item] setItem
-------------------------------------------------------------------------------
function f_stage_reward_item:setItem(data)
	local frame = self.m_componentTable["frame"]
	local p = frame:getPositionInContainer()
	local icon = self.m_componentTable["image"]
...
-------------------------------------------------------------------------------
-- @function [parent=#f_stage_reward_item] setData
-------------------------------------------------------------------------------
function f_stage_reward_item:setData(data)
	self.m_componentTable["piece"]:setVisible(false)
	...
	elseif data["type"] == 12 or data["type"] == 14 then
		local flag = fc.FExtensionsImage:create()
