...
local f_guidelines_702 = class("f_guidelines_702")
f_guidelines_702.__index = f_guidelines_702

-------------------------------------------------
-- @function [parent=#f_guidelines_702] ctor
-------------------------------------------------
function f_guidelines_702:ctor()	
...
-------------------------------------------------------------------------------
-- @function [parent=#f_guidelines_702] setGuidelinesManager
-- 设置新手指引管理
-------------------------------------------------------------------------------
function f_guidelines_702:setGuidelinesManager(manager)	
	self.m_guidelines_manager = manager
end

-------------------------------------------------------------------------------
-- @function [parent=#f_guidelines_702] update
-- 新手指引更新函数
-------------------------------------------------------------------------------
function f_guidelines_702:update(dt)	
...
-------------------------------------------------------------------------------
-- @function [parent=#f_guidelines_702] state_Init
-- 初始化当前状态
-------------------------------------------------------------------------------
function f_guidelines_702:state_Init(dt)
	if g_game.g_current_scene_type == g_game.SCENE_TYPE_GAME then
	
		local gameMap = g_game.g_director:getRunningScene():getGameMap()
		gameMap:setScrollOffset(cc.p(-1500, g_game.g_display.sizeInPixels.height/2))
		--获取地图德胜门
		local deshengmenBtn = g_game.g_director:getRunningScene():getGameMap():getBuilding(7)	
		if deshengmenBtn then
			...
			local path = g_game.g_resourceManager:getBuilding(buildList[7][6])
			...
			buildingComponent:setLabels(buildList[7])
			...
			self.guidelinesPanel:setPositionInContainer(cc.p(stencilSize.width/2-100, stencilSize.height/2-150))
			self.guidelinesPanel:setSwallowsTouches(false)
			self.guidelinesPanel:showText(self.m_guidelines_manager:getGuidelinesText(702))
			self.guidelinesPanel:setGuidlinesPersonImage(self.m_guidelines_manager:getGuidelinesPerson(702), false)
			...
			guidelinesBtn:addHandleOfcomponentEvent(call, g_game.g_f_touch_event.F_TOUCH_UPINSIDE)
...
-------------------------------------------------------------------------------
-- @function [parent=#f_guidelines_702] state_Check
-- 检查是否点击了金銮殿按钮
-------------------------------------------------------------------------------
function f_guidelines_702:state_Check(dt)	
end

-------------------------------------------------------------------------------
-- @function [parent=#f_guidelines_702] state_Finish
-- 结束当前步骤指引
-------------------------------------------------------------------------------
function f_guidelines_702:state_Finish(dt)	
...
-------------------------------------------------------------------------------
-- @function [parent=#f_guidelines_702] state_Exit
-- 退出当前步骤指引
-------------------------------------------------------------------------------
function f_guidelines_702:state_Exit(dt)	
	self.m_finish = true
end

-------------------------------------------------------------------------------
-- @function [parent=#f_guidelines_702] guidelinesFinish
-- 判断当前指引是否结束
-------------------------------------------------------------------------------
function f_guidelines_702:guidelinesFinish()	
	return self.m_finish
end


-------------------------------------------------------------------------------
-- @function [parent=#f_guidelines_702] guidelinesNextIndex
-- 获取下一步指引index
-------------------------------------------------------------------------------
function f_guidelines_702:guidelinesNextIndex()	
	return 3
end

return f_guidelines_702