--[[--
    uc android sdk
--]]--
local f_android_uc_sdk = class("f_android_uc_sdk", function()
    return fc.FSdkManager:getInstance()
end)


function f_android_uc_sdk:ctor()
	...
	-- 登录成功后给平台服务器发送玩家数据
	self.event_send_info_to_sdk = function()
		...
		g_game.g_luaj.callStaticMethod("org/cocos2dx/lua/AppActivity", "ucSdkSubmitExtendData", args, "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;ILjava/lang/String;)V")
	end
	g_game.g_eventManager:registerLuaEvent(g_game.g_f_lua_game_event.F_LUA_SEND_USERINFO_TO_PLANTFORM, self.event_send_info_to_sdk)
end

-------------------------------------------------------------------------------
-- @function [parent=#f_android_uc_sdk] init_sdk
-- 初始化sdk
-------------------------------------------------------------------------------
function f_android_uc_sdk:init_sdk()
	self:setUCVertifiyServer_android(self.m_vertifiy_server["ip"], self.m_vertifiy_server["port"])
	
	local vertifiyUserInfoCallback = function(type, ucid, nickName)
		local loginResult = {}
		if type >= 0 then
			loginResult["result"] = 0
			loginResult["account"] = ucid
			loginResult["userid"] = ucid
			loginResult["error_des"] = ""
			
			self.m_user_info["loginName"] = nickName
			self.m_user_info["sdkUserId"] = ucid
		else
			loginResult["account"] = ""
			loginResult["userid"] = ""
	...
	self:setLoginUCSdkListener_android(vertifiyUserInfoCallback)
end


-------------------------------------------------------------------------------
-- @function [parent=#f_android_uc_sdk] show_sdk_view
-- 显示sdk界面
-------------------------------------------------------------------------------
function f_android_uc_sdk:show_sdk_view()	
	...
	local loginCallback = function(result)
		...
		self.m_user_info["sid"] = resultInfo["sid"]
		
		self:vertifiyUCSid_android(self.m_user_info["sid"])
	end
...
-------------------------------------------------------------------------------
-- @function [parent=#f_android_uc_sdk] change_sdk_user
-- 更改sdk用户
-------------------------------------------------------------------------------
function f_android_uc_sdk:change_sdk_user()
	send_lua_event(g_game.g_f_lua_game_event.F_LUA_NETWORK_LOADING)
...
-------------------------------------------------
-- @function [parent=#f_android_uc_sdk] loadProductsFromServer
-- 使用产品key列表，刷新商店界面
-------------------------------------------------
function f_android_uc_sdk:loadProductsFromServer(itemList)	
...
-------------------------------------------------
-- @function [parent=#f_android_uc_sdk] iapRequestProductsListener
-- 模拟苹果服务器返回列表
-------------------------------------------------
function f_android_uc_sdk:iapRequestProductsListener(data)	
...
-------------------------------------------------
-- @function [parent=#f_android_uc_sdk] checkNativePayment
-- 检查本地存储得收据凭证并进行继续验证
-------------------------------------------------
function f_android_uc_sdk:checkNativePayment()	
...
-------------------------------------------------
-- @function [parent=#f_android_uc_sdk] iapBuyItem
-- 请求购买物品的订单id
-------------------------------------------------
function f_android_uc_sdk:iapBuyItem(data)	
...
-------------------------------------------------
-- @function [parent=#f_android_uc_sdk] buyResultFromUCServer
-- 接收UC服务器返回的购买结果
-------------------------------------------------
function f_android_uc_sdk:buyResultFromUCServer(result)	
...
-------------------------------------------------
-- @function [parent=#f_android_uc_sdk] responseTranscationInfoFromGameServer
-- 接收游戏服务器返回的订单号等信息
-------------------------------------------------
function f_android_uc_sdk:responseTranscationInfoFromGameServer(msg)	
	if msg["result"] == 0 then		
		local buyDoneCallback = function(type)
			self:buyResultFromUCServer(type)
		end		
		
		local userInfo = g_game.g_userInfoManager:getUserInfo()
		--buy item from uc sdk here
		local buyItemJson = {}
		buyItemJson["orderId"] = msg["transactionId"]
		buyItemJson["moneyAmount"] = self.m_current_buy_item_data["info"]["price"]
		buyItemJson["productName"] = self.m_current_buy_item_data["info"]["gold"] .. self.m_current_buy_item_data["info"]["productName"]
		buyItemJson["notifyUrl"] = "http://whwsgm.cmge.com/ChargeServer/android_charge/check_uc_Info.do"
...
-------------------------------------------------
-- @function [parent=#f_android_uc_sdk] requestTransitionInfoFromGameServer
-- 向服务器发送购买请求，并获取订单id
-------------------------------------------------
function f_android_uc_sdk:requestTransitionInfoFromGameServer()		
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
...
-------------------------------------------------
-- @function [parent=#f_android_uc_sdk] requestTransitionVertifyFromGameServer
-- 使用本地缓存的未完成订单，向服务器再次验证
-------------------------------------------------
function f_android_uc_sdk:requestTransitionVertifyFromGameServer(transcationId)	
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
	dataT["roleId"] = userInfo["roleId"]
...
-------------------------------------------------
-- @function [parent=#f_android_uc_sdk] responseBuyResultFromGameServer
-- 从游戏服务器接收购买物品结果
-------------------------------------------------
function f_android_uc_sdk:responseBuyResultFromGameServer(msg)	
