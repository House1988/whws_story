--[[--
    安智 android sdk
--]]--
local f_android_anzhi_sdk = class("f_android_anzhi_sdk", function()
    return fc.FSdkManager:getInstance()
end)


function f_android_anzhi_sdk:ctor()
...
-------------------------------------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] init_sdk
-- 初始化sdk
-------------------------------------------------------------------------------
function f_android_anzhi_sdk:init_sdk()
	self:setAnZhiVertifiyServer_android(self.m_vertifiy_server["ip"], self.m_vertifiy_server["port"])
	...
	local vertifiyUserInfoCallback = function(type, uid)
		...
		if type >= 0 then
			...
			self.m_user_info["loginName"] = self.m_user_info["loginName"]
			self.m_user_info["sdkUserId"] = uid
		else
			loginResult["account"] = ""
			loginResult["userid"] = ""
	...
	self:setLoginAnZhiSdkListener_android(vertifiyUserInfoCallback)
end


-------------------------------------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] show_sdk_view
-- 显示sdk界面
-------------------------------------------------------------------------------
function f_android_anzhi_sdk:show_sdk_view()
	...
	local loginCallback = function(result)
		...
		self.m_user_info["loginName"] = resultInfo["loginName"]
		
		self:vertifiyAnZhiSid_android(self.m_user_info["sid"])
	end
...
-------------------------------------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] change_sdk_user
-- 更改sdk用户
-------------------------------------------------------------------------------
function f_android_anzhi_sdk:change_sdk_user()
	send_lua_event(g_game.g_f_lua_game_event.F_LUA_NETWORK_LOADING )
...
-------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] loadProductsFromServer
-- 使用产品key列表，刷新商店界面
-------------------------------------------------
function f_android_anzhi_sdk:loadProductsFromServer(itemList)	
...
-------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] iapRequestProductsListener
-- 模拟苹果服务器返回列表
-------------------------------------------------
function f_android_anzhi_sdk:iapRequestProductsListener(data)	
...
-------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] checkNativePayment
-- 检查本地存储得收据凭证并进行继续验证
-------------------------------------------------
function f_android_anzhi_sdk:checkNativePayment()	
...
-------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] iapBuyItem
-- 请求购买物品的订单id
-------------------------------------------------
function f_android_anzhi_sdk:iapBuyItem(data)	
...
-------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] buyResultFromAnZhiServer
-- 接收360服务器返回的购买结果
-------------------------------------------------
function f_android_anzhi_sdk:buyResultFromAnZhiServer(result)	
...
-------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] responseTranscationInfoFromGameServer
-- 接收游戏服务器返回的订单号等信息
-------------------------------------------------
function f_android_anzhi_sdk:responseTranscationInfoFromGameServer(msg)	
	if msg["result"] == 0 then		
		local buyDoneCallback = function(type)
			self:buyResultFromAnZhiServer(type)
		end		
		
		local userInfo = g_game.g_userInfoManager:getUserInfo()
		--buy item from anzhi sdk here
		local buyItemJson = {}
...
-------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] requestTransitionInfoFromGameServer
-- 向服务器发送购买请求，并获取订单id
-------------------------------------------------
function f_android_anzhi_sdk:requestTransitionInfoFromGameServer()		
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
...
-------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] requestTransitionVertifyFromGameServer
-- 使用本地缓存的未完成订单，向服务器再次验证
-------------------------------------------------
function f_android_anzhi_sdk:requestTransitionVertifyFromGameServer(transcationId)	
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
	dataT["roleId"] = userInfo["roleId"]
...
-------------------------------------------------
-- @function [parent=#f_android_anzhi_sdk] responseBuyResultFromGameServer
-- 从游戏服务器接收购买物品结果
-------------------------------------------------
function f_android_anzhi_sdk:responseBuyResultFromGameServer(msg)	
