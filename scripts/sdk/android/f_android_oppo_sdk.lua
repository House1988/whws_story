--[[--
    oppo android sdk
--]]--
local f_android_oppo_sdk = class("f_android_oppo_sdk", function()
    return fc.FSdkManager:getInstance()
end)


function f_android_oppo_sdk:ctor()
	...
	local reloginCallback = function(res)
		g_game.startup()
		--数据分析sdk
		send_lua_event(g_game.g_f_lua_game_event.F_LUA_DATA_USER_LOGOUT)
	end
	local jargs = {reloginCallback}
	g_game.g_luaj.callStaticMethod("org/cocos2dx/lua/AppActivity", "setReloginHandler", jargs, "(I)V")
...
-------------------------------------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] init_sdk
-- 初始化sdk
-------------------------------------------------------------------------------
function f_android_oppo_sdk:init_sdk()
	self:setOppoVertifiyServer_android(self.m_vertifiy_server["ip"], self.m_vertifiy_server["port"])
	
	local vertifiyUserInfoCallback = function(type, uid, name)
		local loginResult = {}
		if type >= 0 then
			loginResult["result"] = 0
			loginResult["account"] = uid
			loginResult["userid"] = uid
			loginResult["error_des"] = ""
			
			self.m_user_info["userId"] = uid
			self.m_user_info["userName"] = name
		else
			loginResult["account"] = ""
			loginResult["userid"] = ""
	...
	self:setLoginOppoSdkListener_android(vertifiyUserInfoCallback)
end


-------------------------------------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] show_sdk_view
-- 显示sdk界面
-------------------------------------------------------------------------------
function f_android_oppo_sdk:show_sdk_view()	
	...
	local loginCallback = function(authCode)	
		...
		self.m_user_info["token_secret"] = resultInfo["token_secret"]
		
		self:vertifiyOppoToken_android(self.m_user_info["token"], self.m_user_info["token_secret"])
	end
...
-------------------------------------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] change_sdk_user
-- 更改sdk用户
-------------------------------------------------------------------------------
function f_android_oppo_sdk:change_sdk_user()
	send_lua_event(g_game.g_f_lua_game_event.F_LUA_NETWORK_LOADING)
...
-------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] loadProductsFromServer
-- 使用产品key列表，刷新商店界面
-------------------------------------------------
function f_android_oppo_sdk:loadProductsFromServer(itemList)	
...
-------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] iapRequestProductsListener
-- 模拟苹果服务器返回列表
-------------------------------------------------
function f_android_oppo_sdk:iapRequestProductsListener(data)	
...
-------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] checkNativePayment
-- 检查本地存储得收据凭证并进行继续验证
-------------------------------------------------
function f_android_oppo_sdk:checkNativePayment()	
...
-------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] iapBuyItem
-- 请求购买物品的订单id
-------------------------------------------------
function f_android_oppo_sdk:iapBuyItem(data)	
...
-------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] buyResultFromOppoServer
-- 接收oppo服务器返回的购买结果
-------------------------------------------------
function f_android_oppo_sdk:buyResultFromOppoServer(result)	
...
-------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] responseTranscationInfoFromGameServer
-- 接收游戏服务器返回的订单号等信息
-------------------------------------------------
function f_android_oppo_sdk:responseTranscationInfoFromGameServer(msg)	
	if msg["result"] == 0 then		
		local buyDoneCallback = function(type)
			self:buyResultFromOppoServer(type)
		end		
		
		local userInfo = g_game.g_userInfoManager:getUserInfo()
		--buy item from 360 sdk here
		local buyItemJson = {}
		buyItemJson["itemName"] = self.m_current_buy_item_data["info"]["productName"]
		buyItemJson["itemCount"] = tostring(self.m_current_buy_item_data["info"]["gold"])
		buyItemJson["itemPrice"] = self.m_current_buy_item_data["info"]["price"]*100
		buyItemJson["orderId"] = msg["transactionId"]		
		buyItemJson["checkUrl"] = "http://whwsgm.cmge.com/ChargeServer/android_charge/check_oppo_Info.do"
...
-------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] requestTransitionInfoFromGameServer
-- 向服务器发送购买请求，并获取订单id
-------------------------------------------------
function f_android_oppo_sdk:requestTransitionInfoFromGameServer()
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
...
-------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] requestTransitionVertifyFromGameServer
-- 使用本地缓存的未完成订单，向服务器再次验证
-------------------------------------------------
function f_android_oppo_sdk:requestTransitionVertifyFromGameServer(transcationId)	
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
	dataT["roleId"] = userInfo["roleId"]
...
-------------------------------------------------
-- @function [parent=#f_android_oppo_sdk] responseBuyResultFromGameServer
-- 从游戏服务器接收购买物品结果
-------------------------------------------------
function f_android_oppo_sdk:responseBuyResultFromGameServer(msg)	
