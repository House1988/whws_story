--[[--
    同步推 越狱ios sdk
--]]--
local f_ios_tbt_sdk = class("f_ios_tbt_sdk", function()
    return fc.FSdkManager:getInstance()
end)


function f_ios_tbt_sdk:ctor()
	self.m_sdk_info = {"140336", "", "yn#KiXu8RHTrB4Dayn1Khuj8ReTr4ODa"}
	self.m_tbt_vertifiy_server = {["ip"] = "118.192.76.178",  ["port"] = 20000}
	
	self.m_is_first_open_login = true
	self.m_is_inited = false
	
	self.m_last_login_user = {}
	
	self.m_user_is_login = false
	self.m_login_view_need_show = true
...
-------------------------------------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] init_sdk
-- 初始化sdk
-------------------------------------------------------------------------------
function f_ios_tbt_sdk:init_sdk()
	self:setTBTVertifiyServer(self.m_tbt_vertifiy_server["ip"], self.m_tbt_vertifiy_server["port"])
end


-------------------------------------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] show_sdk_view
-- 显示sdk界面
-------------------------------------------------------------------------------
function f_ios_tbt_sdk:show_sdk_view()
	...
	local loginCallback = function(type, userName, userId)
		...
		if type >= 0 then
			...
			self.m_last_login_user = loginResult
			self.m_is_first_open_login = true
			self.m_user_is_login = true
		else
			loginResult["account"] = ""
			loginResult["userid"] = ""
			if type == -1 then
			...
			elseif type == -14 then
				if g_game.g_game_is_init then					
					...
					self.m_user_is_login = true	
			  		...
			  		if self.m_user_is_login == true then
--						self:showTBTLoginView()
	...
	self:setLoginTBTSdkListener(loginCallback)
	
	
	if g_game.g_game_is_init then
	else
		if self:ppCheckUpdateFinish() then
			self:tbtLogoutLogin()
		else
			if self.m_is_first_open_login then
				if self.m_is_inited == false then
					self:setTBTInit()
					self.m_is_inited = true
				else	
					if self.m_user_is_login == false then
						self:showTBTLoginView()
					else
						send_lua_event_param(g_game.g_f_lua_game_event.F_LUA_SDK_LOGIN_CALLBACK, self.m_last_login_user)						
					end
				end
				self.m_is_first_open_login = false
			else
...
-------------------------------------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] change_sdk_user
-- 更改sdk用户
-------------------------------------------------------------------------------
function f_ios_tbt_sdk:change_sdk_user()
	self.m_is_first_open_login = false
	self:switchTBTUser()
end

-------------------------------------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] open_platform_view
-- 打开用户中心
-------------------------------------------------------------------------------
function f_ios_tbt_sdk:open_platform_view()
	self:showTBTPlatformView()
end
...
-------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] loadProductsFromServer
-- 使用产品key列表，刷新商店界面
-------------------------------------------------
function f_ios_tbt_sdk:loadProductsFromServer(itemList)	
...
-------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] iapRequestProductsListener
-- 模拟苹果服务器返回列表
-------------------------------------------------
function f_ios_tbt_sdk:iapRequestProductsListener(data)	
...
-------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] checkNativePayment
-- 检查本地存储得收据凭证并进行继续验证
-------------------------------------------------
function f_ios_tbt_sdk:checkNativePayment()	
...
-------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] iapBuyItem
-- 请求购买物品的订单id
-------------------------------------------------
function f_ios_tbt_sdk:iapBuyItem(data)	
...
-------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] buyResultFromTBTServer
-- 接受支付界面回调
-------------------------------------------------
function f_ios_tbt_sdk:buyResultFromTBTServer(type)	
	if type == 0 then		
...
-------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] responseTranscationInfoFromGameServer
-- 接收游戏服务器返回的订单号等信息
-------------------------------------------------
function f_ios_tbt_sdk:responseTranscationInfoFromGameServer(msg)	
	if msg["result"] == 0 then		
		local buyDoneCallback = function(type)
			self:buyResultFromTBTServer(type)
		end
		self:setBuyItemTBTSdkListener(buyDoneCallback)
				
		local userInfo = g_game.g_userInfoManager:getUserInfo()
		
		--buy item from itools sdk here
		self:buyItemWithTBTSdk(msg["transactionId"],  self.m_current_buy_item_data["info"]["price"], string.format("%d", msg["serverId"]))
...
-------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] requestTransitionInfoFromGameServer
-- 向服务器发送购买请求，并获取订单id
-------------------------------------------------
function f_ios_tbt_sdk:requestTransitionInfoFromGameServer()		
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
...
-------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] requestTransitionVertifyFromGameServer
-- 使用本地缓存的未完成订单，向服务器再次验证
-------------------------------------------------
function f_ios_tbt_sdk:requestTransitionVertifyFromGameServer(transcationId)	
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
	dataT["roleId"] = userInfo["roleId"]
...
-------------------------------------------------
-- @function [parent=#f_ios_tbt_sdk] responseBuyResultFromGameServer
-- 从游戏服务器接收购买物品结果
-------------------------------------------------
function f_ios_tbt_sdk:responseBuyResultFromGameServer(msg)	
