--[[--
    爱思 越狱ios sdk
--]]--
local f_ios_as_sdk = class("f_ios_as_sdk", function()
    return fc.FSdkManager:getInstance()
end)


function f_ios_as_sdk:ctor()
	self.m_sdk_info = {"69", "", "0e706f71e27b4a5487cea61febd86e3d"}
...
-------------------------------------------------------------------------------
-- @function [parent=#f_ios_as_sdk] init_sdk
-- 初始化sdk
-------------------------------------------------------------------------------
function f_ios_as_sdk:init_sdk()
	self:setAsVertifiyServer(self.m_pp_vertifiy_server["ip"], self.m_pp_vertifiy_server["port"])
end

-------------------------------------------------------------------------------
-- @function [parent=#f_ios_as_sdk] show_sdk_view
-- 显示sdk界面
-------------------------------------------------------------------------------
function f_ios_as_sdk:show_sdk_view()
	...
	self:setLoginAsSdkListener(loginCallback)
	
	self:showAsLoginView()
end

-------------------------------------------------------------------------------
-- @function [parent=#f_ios_as_sdk] change_sdk_user
-- 更改sdk用户
-------------------------------------------------------------------------------
function f_ios_as_sdk:change_sdk_user()
	self:asLogoutLogin()
	self:showAsLoginView()
end
...
-- 打开用户中心
-------------------------------------------------------------------------------
function f_ios_as_sdk:open_user_center()
	self:showAsPlatformView()
end
...
-------------------------------------------------
-- @function [parent=#f_ios_as_sdk] loadProductsFromServer
-- 使用产品key列表，刷新商店界面
-------------------------------------------------
function f_ios_as_sdk:loadProductsFromServer(itemList)	
...
-------------------------------------------------
-- @function [parent=#f_ios_as_sdk] iapRequestProductsListener
-- 模拟苹果服务器返回列表
-------------------------------------------------
function f_ios_as_sdk:iapRequestProductsListener(data)	
...
-------------------------------------------------
-- @function [parent=#f_ios_as_sdk] checkNativePayment
-- 检查本地存储得收据凭证并进行继续验证
-------------------------------------------------
function f_ios_as_sdk:checkNativePayment()	
...
-------------------------------------------------
-- @function [parent=#f_ios_as_sdk] iapBuyItem
-- 请求购买物品的订单id
-------------------------------------------------
function f_ios_as_sdk:iapBuyItem(data)	
...
-------------------------------------------------
-- @function [parent=#f_ios_as_sdk] buyResultFromAsServer
-- 接收As服务器返回的购买结果
-------------------------------------------------
function f_ios_as_sdk:buyResultFromAsServer(type)	
	if type == 0 then	
...
-------------------------------------------------
-- @function [parent=#f_ios_as_sdk] responseTranscationInfoFromGameServer
-- 接收游戏服务器返回的订单号等信息
-------------------------------------------------
function f_ios_as_sdk:responseTranscationInfoFromGameServer(msg)	
	if msg["result"] == 0 then		
		local buyDoneCallback = function(type)
			self:buyResultFromAsServer(type)
		end
		self:setBuyItemAsSdkListener(buyDoneCallback)
				
		local userInfo = g_game.g_userInfoManager:getUserInfo()
		
		--buy item from pp sdk here
		self:buyItemWithAsSdk(self.m_current_buy_item_data["info"]["price"], msg["transactionId"], self.m_current_buy_item_data["info"]["gold"] .. self.m_current_buy_item_data["info"]["productName"], string.format("%d", userInfo["roleId"]), 0)
...
-------------------------------------------------
-- @function [parent=#f_ios_as_sdk] requestTransitionInfoFromGameServer
-- 向服务器发送购买请求，并获取订单id
-------------------------------------------------
function f_ios_as_sdk:requestTransitionInfoFromGameServer()		
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
...
-------------------------------------------------
-- @function [parent=#f_ios_as_sdk] requestTransitionVertifyFromGameServer
-- 使用本地缓存的未完成订单，向服务器再次验证
-------------------------------------------------
function f_ios_as_sdk:requestTransitionVertifyFromGameServer(transcationId)	
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
	dataT["roleId"] = userInfo["roleId"]
...
-------------------------------------------------
-- @function [parent=#f_ios_as_sdk] responseBuyResultFromGameServer
-- 从游戏服务器接收购买物品结果
-------------------------------------------------
function f_ios_as_sdk:responseBuyResultFromGameServer(msg)	
