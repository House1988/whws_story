--[[--
    itools 越狱ios sdk
--]]--
local f_ios_itools_sdk = class("f_ios_itools_sdk", function()
    return fc.FSdkManager:getInstance()
end)


function f_ios_itools_sdk:ctor()
	self.m_sdk_info = {"143", "", "7AA1F1916DB3BC20B1F4648727F5C7EE"}
	self.m_itools_vertifiy_server = {["ip"] = "118.192.76.178",  ["port"] = 20000}
...
-------------------------------------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] init_sdk
-- 初始化sdk
-------------------------------------------------------------------------------
function f_ios_itools_sdk:init_sdk()
	self:setIToolsVertifiyServer(self.m_itools_vertifiy_server["ip"], self.m_itools_vertifiy_server["port"])
end


-------------------------------------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] show_sdk_view
-- 显示sdk界面
-------------------------------------------------------------------------------
function f_ios_itools_sdk:show_sdk_view()
	...
	self:setLoginIToolsSdkListener(loginCallback)
...
-------------------------------------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] change_sdk_user
-- 更改sdk用户
-------------------------------------------------------------------------------
function f_ios_itools_sdk:change_sdk_user()
	self:switchIToolsUser()
end

-------------------------------------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] open_platform_view
-- 打开充值用户管理界面
-------------------------------------------------------------------------------
function f_ios_itools_sdk:open_platform_view()
	self:showIToolsPlatformView()
end
...
-------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] loadProductsFromServer
-- 使用产品key列表，刷新商店界面
-------------------------------------------------
function f_ios_itools_sdk:loadProductsFromServer(itemList)	
...
-------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] iapRequestProductsListener
-- 模拟苹果服务器返回列表
-------------------------------------------------
function f_ios_itools_sdk:iapRequestProductsListener(data)	
...
-------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] checkNativePayment
-- 检查本地存储得收据凭证并进行继续验证
-------------------------------------------------
function f_ios_itools_sdk:checkNativePayment()	
...
-------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] iapBuyItem
-- 请求购买物品的订单id
-------------------------------------------------
function f_ios_itools_sdk:iapBuyItem(data)	
...
-------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] buyResultFromIToolsServer
-- 接受支付界面回调
-------------------------------------------------
function f_ios_itools_sdk:buyResultFromIToolsServer(type)
	if type == 0 then
...
-------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] responseTranscationInfoFromGameServer
-- 接收游戏服务器返回的订单号等信息
-------------------------------------------------
function f_ios_itools_sdk:responseTranscationInfoFromGameServer(msg)	
	if msg["result"] == 0 then		
		local buyDoneCallback = function(type)
			self:buyResultFromIToolsServer(type)
		end
		self:setBuyItemIToolsSdkListener(buyDoneCallback)
				
		local userInfo = g_game.g_userInfoManager:getUserInfo()
		
		--buy item from itools sdk here
		self:buyItemWithIToolsSdk(msg["transactionId"],  self.m_current_buy_item_data["info"]["price"])
		...
		eventData["paymentType"] = "itools支付"
...
-------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] requestTransitionInfoFromGameServer
-- 向服务器发送购买请求，并获取订单id
-------------------------------------------------
function f_ios_itools_sdk:requestTransitionInfoFromGameServer()		
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
...
-------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] requestTransitionVertifyFromGameServer
-- 使用本地缓存的未完成订单，向服务器再次验证
-------------------------------------------------
function f_ios_itools_sdk:requestTransitionVertifyFromGameServer(transcationId)	
	local dataT = {}
	local userInfo = g_game.g_userInfoManager:getUserInfo()
	local dataT = {}
	dataT["roleId"] = userInfo["roleId"]
...
-------------------------------------------------
-- @function [parent=#f_ios_itools_sdk] responseBuyResultFromGameServer
-- 从游戏服务器接收购买物品结果
-------------------------------------------------
function f_ios_itools_sdk:responseBuyResultFromGameServer(msg)	
